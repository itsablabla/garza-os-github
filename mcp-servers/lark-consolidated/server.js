...
function startCategory(cat) {
  ...
  // -u at spawn keeps the auth handler unset (token used as-is); the per-request
  // Authorization header (added below) carries the token past v0.5.1's streamable bug.
  if (process.env.LARK_USER_ACCESS_TOKEN && !cat.domain) args.push('-u', process.env.LARK_USER_ACCESS_TOKEN);
